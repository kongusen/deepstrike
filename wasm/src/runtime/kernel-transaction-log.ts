...
export async function kernelRecordDigest(value: unknown): Promise<string> {
...
export async function createKernelOperationGenesis(
  ...
  const body: KernelOperationGenesisBody = { record_version: KERNEL_LOG_RECORD_VERSION, ...input }
...
export async function verifyKernelOperationGenesis(genesis: KernelOperationGenesis): Promise<void> {
...
export async function verifyKernelTransaction(transaction: KernelTransaction): Promise<void> {
...
export async function verifyKernelTransactionStream(
  genesis: KernelOperationGenesis,
  transactions: readonly KernelTransaction[],
): Promise<KernelOperationCursor> {
...
function validateGenesisBody(genesis: KernelOperationGenesisBody): void {
  ...
  if (!Number.isSafeInteger(genesis.default_policy_version) || genesis.default_policy_version <= 0) throw new KernelLogIntegrityError("kernel genesis default_policy_version must be a positive safe integer")
}

function validateTransactionBody(transaction: KernelTransactionBody): void {
  if (transaction.record_version !== KERNEL_LOG_RECORD_VERSION) throw new KernelLogIntegrityError("unsupported kernel transaction record version")
  if (!transaction.operation_id) throw new KernelLogIntegrityError("kernel transaction operation_id is required")
  if (!Number.isSafeInteger(transaction.step_seq) || transaction.step_seq <= 0) throw new KernelLogIntegrityError("kernel transaction step_seq must be a positive safe integer")
  if (!Number.isSafeInteger(transaction.base_generation) || transaction.base_generation < 0) throw new KernelLogIntegrityError("kernel transaction base_generation must be a non-negative safe integer")
  if (!transaction.previous_transaction_digest) throw new KernelLogIntegrityError("kernel transaction previous_transaction_digest is required")
}
