...
function providerStopReason(value: unknown): string | undefined {
  ...
  return normalizeProviderStopReason(value)
}
...
export class CanonicalRunnerRuntime {
  ...
  private appendBase(): WorkflowNodeIdBase {
    return this.workflowNodeCount ?? `wf-append-${crypto.randomUUID().slice(0, 8)}`
  }
  ...
  private canonicalSignal(event: Record<string, unknown>): Record<string, unknown> {
    ...
    return {
      kind: "deliver_signal",
      delivery_id: String(event.delivery_id ?? crypto.randomUUID()),
