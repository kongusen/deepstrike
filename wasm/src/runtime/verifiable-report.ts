...
export type ForkPlan = VerifiableForkManifest
...
/** Create the adapter from the lazily loaded WASM binding. */
export async function createNativeVerifiableRuntimeAdapter(): Promise<VerifiableRuntimeAdapter> {
  const kernel = await getKernel()
  return createVerifiableRuntimeAdapter(kernel.verifiableOperationJson)
}

/** Storage-neutral framework handle. Store access remains in the host adapter. */
