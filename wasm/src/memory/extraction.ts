...
export async function extractSessionMemories(provider: LLMProvider, session: SessionData,
  scope: MemoryScope, systemPrompt?: string): Promise<MemoryRecord[]> {
  ...
  const context = {
    ...
    turns: [{ role: "user" as const, content: `${transcript}\n\nReturn {"memories":[{"name":"stable-kebab-key","kind":"user|feedback|project|reference","content":"fact","description":"why durable","confidence":0.0,"links":[],"pinned":false,"ttl_days":null,"evidence_refs":[]}]} with at most 10 items. Return {"memories":[]} when nothing is durable.`, toolCalls: [] } satisfies Message],
  }
  let output = ""
...
export function parseExtractedMemories(output: string, session: SessionData, scope: MemoryScope): MemoryRecord[] {
  let value: unknown
  try { value = JSON.parse(output.trim().replace(/^```(?:json)?\s*/i, "").replace(/\s*```$/, "")) } catch { return [] }
  ...
  for (const raw of (value as { memories: unknown[] }).memories.slice(0, 10)) {
    ...
    const kind = typeof draft.kind === "string" && KINDS.has(draft.kind as MemoryKind) ? draft.kind as MemoryKind : undefined
    ...
    records.push({
      ...
      provenance: { session_id: session.sessionId, author: "extraction", trust: "untrusted", evidence_refs: Array.isArray(draft.evidence_refs) ? draft.evidence_refs.filter((ref): ref is string => typeof ref === "string") : [] },
