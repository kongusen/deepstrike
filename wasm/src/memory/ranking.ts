...
/** Deterministic lexical relevance, then recency, then insertion order. */
export function rankMemories<T>(query: string, candidates: Array<RankableMemory<T>>, topK: number): T[] {
  ...
  return candidates
    .map(candidate => {
      ...
      for (const term of queryTerms) if (candidateTerms.has(term)) lexicalMatches++
      return { ...candidate, lexicalMatches }
    })
    .filter(candidate => queryTerms.size === 0 || candidate.lexicalMatches > 0)
    .sort((a, b) => b.lexicalMatches - a.lexicalMatches
