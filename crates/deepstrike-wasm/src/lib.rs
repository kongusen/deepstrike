...
impl Tournament {
    #[wasm_bindgen(constructor)]
    pub fn new(entrants: Vec<String>) -> Result<Tournament, JsValue> {
        let inner = RustTournament::new(entrants).map_err(|e| JsValue::from_str(&e.to_string()))?;
        Ok(Self { inner })
    }
    ...
    #[wasm_bindgen(js_name = feedRound)]
    pub fn feed_round(&mut self, winners: Vec<String>) -> Result<TournamentAction, JsValue> {
        let action = self
            .inner
            .feed_round(winners)
...
fn loop_stop_reason_label(reason: RustStopReason) -> String {
    match reason {
...
fn loop_action_from_rust(action: RustLoopAction) -> LoopAction {
    match action {
        ...
        RustLoopAction::Done {
            ...
            round: None,
            rounds_used: Some(rounds_used),
            reason: Some(loop_stop_reason_label(reason)),
        },
    }
}
...
impl LoopUntilDone {
    #[wasm_bindgen(constructor)]
    pub fn new(conditions: Vec<StopConditionSpec>) -> Result<LoopUntilDone, JsValue> {
        ...
        for c in conditions {
            let cond = match c.kind.as_str() {
                ...
                "maxRounds" => RustStopCondition::MaxRounds(c.max_rounds.ok_or_else(|| {
                    JsValue::from_str("maxRounds condition requires a maxRounds value")
                })?),
                other => {
