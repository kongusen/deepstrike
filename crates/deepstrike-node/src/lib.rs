...
use deepstrike_core::orchestration::tournament::{
    Match as RustMatch, Tournament as RustTournament, TournamentAction as RustTournamentAction,
};
...
// ===========================================================================
// Tournament — single-elimination bracket with pairwise comparative judging.
// ===========================================================================
...
pub struct TournamentAction {
    ...
    pub matches: Option<Vec<TournamentMatch>>,
    pub winner: Option<String>,
    pub rounds_used: Option<u32>,
}

fn tournament_action_from_rust(action: RustTournamentAction) -> TournamentAction {
    match action {
        RustTournamentAction::JudgeRound { round, matches } => TournamentAction {
            kind: "judgeRound".into(),
            round: Some(round),
            matches: Some(
                matches
                    .into_iter()
                    .map(|m: RustMatch| TournamentMatch {
                        id: m.id,
                        left: m.left,
                        right: m.right,
                    })
...
impl Tournament {
    #[napi(constructor)]
    pub fn new(entrants: Vec<String>) -> Result<Self> {
        let inner = RustTournament::new(entrants)
    ...
    pub fn start(&mut self) -> TournamentAction {
        tournament_action_from_rust(self.inner.start())
    }
    ...
    #[napi]
    pub fn feed_round(&mut self, winners: Vec<String>) -> Result<TournamentAction> {
        let action = self
            .inner
            .feed_round(winners)
            .map_err(|e| Error::new(Status::InvalidArg, e.to_string()))?;
        Ok(tournament_action_from_rust(action))
    }
...
// ===========================================================================
// LoopUntilDone — drive an agent round after round until a stop predicate fires.
// ===========================================================================
...
fn stop_reason_label(reason: RustStopReason) -> String {
    match reason {
        RustStopReason::NoNewFindings => "noNewFindings",
        RustStopReason::NoErrors => "noErrors",
        RustStopReason::MaxRounds => "maxRounds",
    }
    .into()
}

fn loop_action_from_rust(action: RustLoopAction) -> LoopAction {
    match action {
        RustLoopAction::Spawn { round } => LoopAction {
            kind: "spawn".into(),
            round: Some(round),
            rounds_used: None,
            reason: None,
...
impl LoopUntilDone {
    #[napi(constructor)]
    pub fn new(conditions: Vec<StopConditionSpec>) -> Result<Self> {
        let mut rust_conditions = Vec::with_capacity(conditions.len());
        for c in conditions {
            let cond = match c.kind.as_str() {
                "noNewFindings" => RustStopCondition::NoNewFindings,
                "noErrors" => RustStopCondition::NoErrors,
                "maxRounds" => RustStopCondition::MaxRounds(c.max_rounds.ok_or_else(|| {
                    napi::Error::from_reason("maxRounds condition requires a maxRounds value")
                })?),
                other => {
