...
pub enum GenEvalAction {
    Generate { iteration: u32, feedback: Option<String> },
    Done { iterations_used: u32, final_score: f64 },
}
...
mod tests {
    ...
    fn passes_on_high_score() {
        ...
        ge.start();
        let action = ge.step(EvalResult { score: 0.9, feedback: None, pass: false });
        assert!(matches!(action, GenEvalAction::Done { .. }));
    }

    #[test]
    fn iterates_on_low_score() {
        let mut ge = GenEvalLoop::new(GenEvalConfig { max_iterations: 3, quality_threshold: 0.8 });
        ge.start();
        let action = ge.step(EvalResult { score: 0.3, feedback: Some("improve X".into()), pass: false });
        assert!(matches!(action, GenEvalAction::Generate { iteration: 2, .. }));
    }

    #[test]
    fn stops_at_max_iterations() {
        let mut ge = GenEvalLoop::new(GenEvalConfig { max_iterations: 2, quality_threshold: 0.9 });
        ge.start();
        ge.step(EvalResult { score: 0.3, feedback: None, pass: false });
        let action = ge.step(EvalResult { score: 0.5, feedback: None, pass: false });
        assert!(matches!(action, GenEvalAction::Done { iterations_used: 2, .. }));
    }
}
