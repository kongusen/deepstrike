...
#[test]
fn fixed_context_overflow_fails_closed_before_provider_call() {
    let mut sm = LoopStateMachine::new(SchedulerBudget {
        max_tokens: 1,
        ..SchedulerBudget::default()
    });

    let action = sm.start(RuntimeTask::new("a task whose state cannot fit"));

    assert!(matches!(
        action,
        LoopAction::Done {
            result: LoopResult {
                termination: TerminationReason::ContextOverflow,
                ..
            }
        }
    ));
    assert!(
        sm.take_observations().iter().any(|observation| matches!(
            observation,
...
fn resume_after_preload_does_not_page_in_pending_memory() {
    ...
    assert!(
        ...
        "must still resume the pending call"
    );
    ...
    assert!(
        sm.observations.is_empty()
            || !sm.observations.iter().any(|o| {
                serde_json::to_string(o)
                    .unwrap_or_default()
                    .contains("page_in_requested")
            })
    );
}
...
fn start_places_user_message_in_history_not_signals() {
    ...
    assert!(
        sm.ctx.partitions.signals.is_empty(),
        "signals should stay empty at start"
    );
}
...
fn critical_signal_goes_to_signals_not_history() {
    ...
    assert!(
        !sm.ctx
            ...
            .directives
            .iter()
            .any(|directive| directive.contains("fire"))
    );
    assert_eq!(sm.ctx.partitions.history.messages.len(), history_len_before);
}

#[test]
fn normal_signal_wakes_a_ready_nonterminal_loop() {
    ...
    assert_eq!(sm.signal_router.depth(), 0);
}

#[test]
fn terminal_signal_is_queued_and_reported_pending() {
    use crate::types::signal::{SignalSource, SignalType, Urgency};

    let mut sm = sm();
    sm.start(RuntimeTask::new("finish first"));
    sm.feed(LoopEvent::Complete);
    let signal = RuntimeSignal::new(
        SignalSource::Gateway,
        SignalType::Job,
        Urgency::Normal,
...
#[test]
fn queue_displacement_and_expiration_are_observable() {
    use crate::types::signal::{SignalSource, SignalType, Urgency};

    let mut sm = sm();
    sm.set_signal_policy(2, Some(10), false);
    sm.start(RuntimeTask::new("finish first"));
    sm.feed(LoopEvent::Complete);

    let old = RuntimeSignal::new(SignalSource::Cron, SignalType::Event, Urgency::Low, "old")
        .with_timestamp(10);
    ...
    assert!(sm.take_observations().iter().any(|observation| matches!(
        ...
        KernelObservation::SignalDisplaced {
            ...
            displaced_signal_id,
            ..
        } if admitted_signal_id == &admitted_id && displaced_signal_id == &newest_id
    )));

    sm.set_observed_time(30);
...
#[test]
fn queued_signal_expires_at_turn_boundary_without_another_ingest() {
    use crate::types::signal::{SignalSource, SignalType, Urgency};

    let mut sm = sm();
    sm.set_signal_policy(2, Some(10), false);
    sm.start(RuntimeTask::new("wait for provider"));
    let stale = RuntimeSignal::new(
        SignalSource::Cron,
        SignalType::Event,
        Urgency::Normal,
        "stale queued work",
    ...
    assert_eq!(sm.signal_router.depth(), 0);
    assert!(sm.ctx.partitions.signals.is_empty());
    assert!(
        sm.take_observations()
            .iter()
            .any(|observation| matches!(observation, KernelObservation::SignalExpired { .. }))
...
#[test]
fn coalesced_queue_entry_renders_its_deterministic_count() {
    use crate::types::signal::{SignalSource, SignalType, Urgency};

    let mut sm = sm();
    sm.start(RuntimeTask::new("wait for a batch"));
    ...
    let second = RuntimeSignal::new(
        ...
        SignalType::Event,
        Urgency::Normal,
        "newer summary is not substituted",
    )
    .with_timestamp(20)
    .with_coalesce("updates");
    ...
    assert!(
        sm.ctx
            ...
            .signals
            .iter()
            .any(|summary| summary.contains("[x2] batched update"))
    );
}
...
fn interrupt_now_preempts_awaited_subagent() {
    ...
    assert!(
        sm.is_suspended(),
        "request alone does not commit preemption"
    );
    assert!(
        !sm.take_observations()
            .iter()
    ...
    assert!(
        ...
        "result reclaims the root"
    );
    assert!(
        !sm.is_suspended(),
        "confirmed preemption cleared SubAgentAwait"
    );
    ...
    assert!(
        obs.iter().any(|o| matches!(
            ...
            KernelObservation::WorkflowCompleted { node_outcomes, .. }
                if node_outcomes.iter().any(|outcome| {
                    outcome.node_id == "wf-node0"
                        && outcome.status
                            == crate::orchestration::workflow::WorkflowNodeStatus::Failed
                })
        )),
...
fn high_urgency_interrupt_does_not_preempt() {
    // High (not Critical) signal while busy → soft Interrupt: expose the signal to the next
    // provider request, but do NOT turn it into a standing directive or preempt the running child.
    use crate::types::agent::{AgentIdentity, AgentRole, AgentRunSpec};
    use crate::types::signal::{SignalSource, SignalType, Urgency};
    let mut sm = sm();
    sm.start(RuntimeTask::new("parent"));
    ...
    assert!(
        sm.ctx
            ...
            .signals
            .iter()
            .any(|signal| signal.contains("handle soon"))
    );
    assert!(
        !sm.ctx
            .partitions
            ...
            .directives
            .iter()
            .any(|directive| directive.contains("handle soon"))
    );
}

#[test]
fn signal_is_consumed_after_provider_commit_and_does_not_survive_renewal() {
    // A signal is rendered for exactly one provider request. ProviderResult commits that request
    // and consumes the delivered prefix; subsequent projections and renewal must not resurrect it.
    use crate::types::signal::{SignalSource, SignalType, Urgency};
    let mut sm = sm();
    ...
    assert!(
        context
            ...
            .is_some_and(|text| text.contains("migration files")),
        "the correlated provider request must contain the signal"
    );
    ...
    assert!(
        !sm.ctx
            ...
            .directives
            .iter()
            .any(|directive| directive.contains("migration files"))
    );
}
...
fn active_skill_gates_exposed_tools_with_stable_core() {
    ...
    debug.allowed_tools = vec![
        ...
        compact_str::CompactString::new("grep"),
    ];
    ...
    match sm.emit_call_llm() {
        LoopAction::CallLLM { tools, .. } => {
            ...
            assert!(
                names.contains(&SKILL_TOOL_NAME),
                "meta-tool exempt: {names:?}"
            );
...
fn gating_is_byte_stable_within_an_epoch() {
    ...
    assert!(
        n3.contains(&"grep".to_string()),
        "union now includes review's tools: {n3:?}"
    );
}
    ...
    match sm.start(RuntimeTask::new("do the task")) {
        LoopAction::CallLLM { tools, .. } => {
            ...
            assert!(
                ...
                "search should be exposed: {names:?}"
            );
            assert!(
                !names.contains(&"write"),
                "write must be gated out: {names:?}"
            );
            assert!(
                !names.contains(&"bash"),
                "bash must be gated out: {names:?}"
            );
        }
        _ => panic!("expected CallLLM"),
    }
...
fn force_compact_emits_page_out_when_archived() {
    ...
    assert!(
        matches!(action, LoopAction::ArchivePageOut { archived, tier, .. } if !archived.is_empty() && tier == "semantic")
    );
}
...
fn knowledge_upsert_applies_at_renewal_boundary() {
    ...
    assert_eq!(
        ...
        1,
        "keyed identity carried, not duplicated"
    );
}
...
fn recover_attempt_cap_is_bounded() {
    ...
    assert!(
        terminated,
        "recovery ladder must terminate under repeated overflow"
    );
}
...
fn mount_unmount_capability_emits_observation() {
    ...
    let desc = crate::types::capability::CapabilityDescriptor::tool(schema).with_version("1.0.0");
...
fn ask_user_enters_suspended_without_execute_tools() {
    ...
    assert!(
        obs.iter()
            .any(|o| matches!(o, KernelObservation::Suspended { .. }))
    );
    assert!(
        !obs.iter()
...
fn spawn_sub_agent_suspends_until_completed() {
    ...
    assert_eq!(
        sm.agent_process("child").expect("process").state,
...
fn memory_write_quota_rate_limits_within_window() {
    ...
    let req = MemoryRecord {
        record_id: "record-m".into(),
        scope: MemoryScope::new("tenant-test", "quota-tests"),
        name: "m".into(),
...
fn task_table_holds_root_after_start() {
    ...
    assert_eq!(
        sm.task_table().get("root").unwrap().state,
        TaskLifecycle::Ready
    );
    sm.start(RuntimeTask::new("hi"));
...
fn workflow_fanout_spawns_batch_then_synthesizes() {
    ...
    let final_action = feed_workflow(
        &mut sm,
        LoopEvent::SubAgentCompleted {
...
fn submit_workflow_nodes_appends_and_spawns_mid_run() {
    ...
    let action = submit_workflow_nodes_started(
        &mut sm,
...
#[test]
fn malformed_workflow_submission_emits_typed_rejection() {
    use crate::orchestration::workflow::{WorkflowNode, WorkflowSpec};
    use crate::types::agent::AgentRole;
    ...
    sm.start(RuntimeTask::new("parent"));
    sm.take_observations();
    load_workflow_started(
        &mut sm,
        WorkflowSpec::new(vec![WorkflowNode::new(
            RuntimeTask::new("root"),
            AgentRole::Implement,
        )]),
        "sess",
    ...
    submit_workflow_nodes_started(
        &mut sm,
        vec![
            WorkflowNode::new(RuntimeTask::new("bad"), AgentRole::Implement)
...
fn submit_workflow_flattens_onto_active_workflow() {
    ...
    assert!(
        ...
        "flattened node spawned in the same DAG"
    );
...
fn zero_concurrency_quota_denies_workflow_spawns_instead_of_stalling() {
    ...
    assert_eq!(
        count_spawned(&obs),
        0,
        "nothing spawns under a zero-slot pool"
    );
    let completed = obs.iter().find_map(|o| match o {
        KernelObservation::WorkflowCompleted { node_outcomes, .. } => Some(node_outcomes.clone()),
        _ => None,
    });
    if let Some(node_outcomes) = completed {
        assert_eq!(node_outcomes.len(), 1);
        assert_eq!(node_outcomes[0].node_id, "wf-node0");
        assert_eq!(
            node_outcomes[0].status,
            crate::orchestration::workflow::WorkflowNodeStatus::Failed,
            "node denied, not silently skipped"
        );
    }
}
...
fn quarantined_node_output_is_labeled_crossing_into_trusted_context() {
    ...
    assert!(
        ...
        "quarantined ReadOnly node spawns"
    );
...
fn quarantined_node_with_write_isolation_is_denied_in_kernel() {
    ...
    let spec = WorkflowSpec::new(vec![
        WorkflowNode::new(
            RuntimeTask::new("read untrusted webpage"),
            AgentRole::Explore,
        )
    ...
    assert!(
        obs.iter()
            ...
            || sm
                ...
                .signals
                .iter()
                .any(|s| s.to_lowercase().contains("quarantine")),
...
fn loop_node_reruns_then_unblocks_dependent_via_drive_workflow() {
    ...
    assert!(matches!(
        feed_workflow(
            &mut sm,
            LoopEvent::SubAgentCompleted {
                result: wf_completed("wf-node0-i0")
            }
        ),
        LoopAction::AwaitingResume
    ));
    assert_eq!(count_spawned(&sm.take_observations()), 1);
    assert!(
        sm.agent_process("wf-node0-i1").is_some(),
        "second iteration spawned"
    );
    ...
    assert!(matches!(
        feed_workflow(
            &mut sm,
            LoopEvent::SubAgentCompleted {
                result: wf_completed("wf-node0-i1")
            }
        ),
        LoopAction::AwaitingResume
    ));
    ...
    assert!(
        sm.agent_process("wf-node1").is_some(),
        "dependent spawns after the loop ends"
    );
...
fn loop_node_stops_early_on_loop_continue_false() {
    ...
    feed_workflow(
        &mut sm,
        LoopEvent::SubAgentCompleted {
            result: wf_completed_stop("wf-node0-i1"),
        },
    );
    assert_eq!(count_spawned(&sm.take_observations()), 1);
    assert!(
        sm.agent_process("wf-node1").is_some(),
        "early stop promoted the dependent"
    );
    assert!(
        sm.agent_process("wf-node0-i2").is_none(),
        "no third iteration ran"
    );
...
fn classify_node_routes_to_chosen_branch_and_prunes_others() {
    ...
    let spec = WorkflowSpec::new(vec![
        WorkflowNode::new(RuntimeTask::new("classify ticket"), AgentRole::Plan).with_classify(
            vec![
                ClassifyBranch {
                    label: "bug".into(),
                    nodes: vec![1],
    ...
    feed_workflow(
        &mut sm,
        LoopEvent::SubAgentCompleted {
            result: wf_completed_branch("wf-node0", "bug"),
        },
    );
    assert_eq!(count_spawned(&sm.take_observations()), 1);
    assert!(sm.agent_process("wf-node1").is_some(), "chosen branch runs");
    assert!(
        sm.agent_process("wf-node2").is_none(),
        "other branch pruned"
    );
...
fn tournament_node_drives_bracket_end_to_end_via_drive_workflow() {
    ...
    assert!(
        sm.agent_process("wf-node0").is_none(),
        "controller spawns no agent of its own"
    );
    for id in ["wf-node2", "wf-node3", "wf-node4", "wf-node5"] {
        assert!(sm.agent_process(id).is_some(), "{id} entrant spawned");
    }
    assert!(
        last_batch_spawns(&obs)
            .iter()
            .all(|n| n.judge_match.is_none()),
        "entrants aren't judges"
    );
    ...
    assert_eq!(
        count_spawned(&r1),
        2,
        "two round-1 judges (wf-node6, wf-node7)"
    );
    let r1_matches: Vec<_> = last_batch_spawns(&r1)
        .iter()
        .filter_map(|n| n.judge_match.clone())
        .collect();
    ...
    assert!(
        sm.agent_process("wf-node1").is_none(),
        "dependent waits for the whole bracket"
    );
    ...
    feed_workflow(
        &mut sm,
        LoopEvent::SubAgentCompleted {
            result: wf_completed_winner("wf-node6", "wf-node2"),
        },
    );
    assert_eq!(
        count_spawned(&sm.take_observations()),
        0,
        "final waits for both round-1 judges"
    );
    feed_workflow(
        &mut sm,
        LoopEvent::SubAgentCompleted {
            result: wf_completed_winner("wf-node7", "wf-node4"),
        },
    );
    let r2 = sm.take_observations();
    assert_eq!(count_spawned(&r2), 1, "one final judge (wf-node8)");
    let r2_match = last_batch_spawns(&r2)[0]
        .judge_match
        .clone()
        .expect("final judge match");
    ...
    feed_workflow(
        &mut sm,
        LoopEvent::SubAgentCompleted {
            result: wf_completed_winner("wf-node8", "wf-node4"),
        },
    );
    assert_eq!(
        count_spawned(&sm.take_observations()),
        1,
        "dependent spawns after the bracket"
    );
    assert!(
        sm.agent_process("wf-node1").is_some(),
        "the 'ship the winner' dependent"
    );
...
fn quarantined_node_read_only_is_allowed() {
    ...
    assert_eq!(
        count_spawned(&sm.take_observations()),
        1,
        "read-only quarantined node spawns"
    );
    assert!(sm.agent_process("wf-node0").is_some());
}
...
fn workflow_run_queue_unblocks_dependents_per_node() {
    ...
    assert_eq!(
        count_spawned(&obs),
        1,
        "D unblocks on A alone, not waiting for B"
    );
    assert!(
        sm.agent_process("wf-node3").is_some(),
        "D (node 3) is running"
    );
...
fn fuse_drive_turn(sm: &mut LoopStateMachine, args: serde_json::Value) -> LoopAction {
    let action = sm.feed(LoopEvent::LLMResponse {
        message: fuse_tool_turn("set_title", args),
    });
    if matches!(action, LoopAction::ExecuteTools { .. }) {
        return sm.feed(LoopEvent::ToolResults {
...
fn repeat_fuse_denies_at_threshold_and_feeds_directive_back() {
    ...
    for _ in 0..2 {
        ...
        assert!(
            ...
            "turn should complete and re-call"
        );
        assert!(
            !sm.observations
                .iter()
...
fn repeat_fuse_escalates_to_no_progress_termination() {
    ...
    match a {
        LoopAction::CallLLM { tools, .. } => {
            assert!(tools.is_empty(), "final turn must strip tools")
        }
...
fn criteria_gate_injects_one_self_check_before_completed() {
    ...
    assert!(
        matches!(a, LoopAction::CallLLM { .. }),
        "gate must re-call, got {a:?}"
    );
    assert!(!sm.is_terminal());
    assert!(
        sm.observations
            .iter()
            .any(|o| matches!(o, KernelObservation::CriteriaGateFired { .. }))
    );
    assert!(
        sm.ctx
            .partitions
            .signals
            .iter()
            .any(|s| s.contains("[CRITERIA CHECK]") && s.contains("tests pass")),
...
fn pace_sleep_is_clamped_and_records_coercion() {
    ...
    let action = sm.feed(LoopEvent::LLMResponse {
        message: pace_turn("sleep", Some(5)),
    });
    assert!(
        matches!(action, LoopAction::CallLLM { ref tools, .. } if tools.is_empty()),
...
fn zero_round_grant_stops_before_provider_dispatch() {
    ...
    run_spec.loop_round = Some(LoopRoundSpec {
        max_rounds: Some(3),
        ..Default::default()
    });
    sm.run_spec = Some(run_spec);
    ...
    assert_eq!(
        sm.turn, 0,
        "no provider turn may start without round capacity"
    );
...
fn pace_stop_routes_through_criteria_gate_then_honors_the_redecision() {
    ...
    assert!(
        sm.ctx
            ...
            .signals
            .iter()
            .any(|s| s.contains("[CRITERIA CHECK]"))
    );
    assert!(
        sm.take_observations()
            .iter()
...
fn pace_emits_local_round_number() {
    ...
    let paced = obs.iter().find_map(|o| match o {
        KernelObservation::RoundPaced {
            round, decision, ..
        } => Some((*round, decision.clone())),
        _ => None,
    });
    let (round, decision) = paced.expect("RoundPaced emitted");
    assert_eq!(
        round, 1,
        "round numbering is local to the granted operation"
    );
...
fn entropy_drive_turn(
    ...
    let a = sm.feed(LoopEvent::LLMResponse {
        message: fuse_tool_turn("step", args),
    });
    assert!(
        matches!(a, LoopAction::ExecuteTools { .. }),
        "expected ExecuteTools, got {a:?}"
    );
...
fn entropy_sample_reflects_repeats_and_failures() {
    ...
    assert!(
        repeat > 0.0,
        "identical-call streak must register, got {repeat}"
    );
    assert!(
        (failures - 1.0).abs() < 1e-9,
        "all results errored, got {failures}"
    );
...
fn entropy_watch_alerts_once_and_respects_optin() {
    ...
    for _ in 0..4 {
        ...
        assert!(
            !obs.iter()
                .any(|o| matches!(o, KernelObservation::EntropyAlert { .. })),
    ...
    for _ in 0..4 {
        ...
        alerts += obs
            .iter()
            .filter(|o| matches!(o, KernelObservation::EntropyAlert { .. }))
            .count();
    }
    assert_eq!(
        alerts, 1,
        "hysteresis must gate re-fires while the score stays hot"
    );
}
...
fn entropy_watch_notify_model_routes_a_heartbeat_signal() {
    ...
    for _ in 0..4 {
            ...
            .any(|o| matches!(o, KernelObservation::EntropyAlert { .. }))
        {
            ...
            assert!(
                !obs.iter()
                    .any(|o| matches!(o, KernelObservation::SignalDeliveryDisposed { .. }))
            );
            assert!(
                sm.ctx
                    .partitions
...
fn entropy_rollbacks_accrue_into_the_next_completed_turn() {
    ...
    assert!(
        entropy_sample_of(&rolled).is_none(),
        "rolled-back turn has no boundary sample"
    );
