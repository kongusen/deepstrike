...
use deepstrike_core::orchestration::loop_until_done::{
    LoopAction as RustLoopAction, LoopConfig as RustLoopConfig, LoopUntilDone as RustLoopUntilDone,
    RoundReport as RustRoundReport, StopCondition as RustStopCondition,
    StopReason as RustStopReason,
};
use deepstrike_core::orchestration::tournament::{
    Tournament as RustTournament, TournamentAction as RustTournamentAction,
};
...
/// - `"judge_round"` → `round`, `matches`
/// - `"done"`        → `winner`, `rounds_used`
#[pyclass]
#[derive(Clone)]
struct TournamentAction {
    ...
    round: Option<u32>,
    #[pyo3(get)]
    matches: Option<Vec<TournamentMatch>>,
    #[pyo3(get)]
    winner: Option<String>,
    #[pyo3(get)]
    rounds_used: Option<u32>,
}
...
impl TournamentAction {
    fn __repr__(&self) -> String {
        format!(
            "TournamentAction(kind={:?}, winner={:?})",
            self.kind, self.winner
        )
    }
}

impl TournamentAction {
    fn from_rust(a: RustTournamentAction) -> Self {
        match a {
            RustTournamentAction::JudgeRound { round, matches } => Self {
                kind: "judge_round".into(),
                round: Some(round),
                matches: Some(
                    matches
                        .into_iter()
                        .map(|m| TournamentMatch {
                            id: m.id,
                            left: m.left,
                            right: m.right,
                        })
...
///
/// 1. `Tournament(entrants)` (raises ValueError if empty)
/// 2. `start()` → `"judge_round"` (or `"done"` for a single entrant)
/// 3. Run one fresh-context judge per `action.matches`, collect each winner
/// 4. `feed_round(winners)` → next `"judge_round"` or `"done"`
#[pyclass]
struct Tournament {
    inner: RustTournament,
}

#[pymethods]
impl Tournament {
    #[new]
    fn new(entrants: Vec<String>) -> PyResult<Self> {
        let inner =
            RustTournament::new(entrants).map_err(|e| PyValueError::new_err(e.to_string()))?;
        Ok(Self { inner })
    }

    fn start(&mut self) -> TournamentAction {
        TournamentAction::from_rust(self.inner.start())
    }

    fn feed_round(&mut self, winners: Vec<String>) -> PyResult<TournamentAction> {
        let action = self
            .inner
            .feed_round(winners)
            .map_err(|e| PyValueError::new_err(e.to_string()))?;
        Ok(TournamentAction::from_rust(action))
    }
...
impl RoundReport {
    #[new]
    fn new(new_findings: u32, errors: u32) -> Self {
        Self {
            new_findings,
            errors,
        }
...
impl LoopAction {
    fn __repr__(&self) -> String {
        format!(
            "LoopAction(kind={:?}, round={:?}, reason={:?})",
            self.kind, self.round, self.reason
        )
    }
}

fn stop_reason_label(reason: RustStopReason) -> String {
    match reason {
        RustStopReason::NoNewFindings => "no_new_findings",
        RustStopReason::NoErrors => "no_errors",
        RustStopReason::MaxRounds => "max_rounds",
    }
    .into()
}
...
impl LoopAction {
    fn from_rust(a: RustLoopAction) -> Self {
        match a {
            RustLoopAction::Spawn { round } => Self {
                kind: "spawn".into(),
                round: Some(round),
                rounds_used: None,
                reason: None,
...
/// Loop-until-done state machine. `conditions` is a list of dicts/objects with a `kind`
/// (`"no_new_findings"` | `"no_errors"` | `"max_rounds"`) and, for `"max_rounds"`, a
/// `max_rounds` int. A `max_rounds` backstop is injected automatically if none is given.
///
/// 1. `LoopUntilDone(conditions)`
/// 2. `start()` → `"spawn"` (round 1)
/// 3. Run the worker, then `feed(RoundReport(new_findings, errors))` → `"spawn"` or `"done"`
#[pyclass]
struct LoopUntilDone {
    inner: RustLoopUntilDone,
}

#[pymethods]
impl LoopUntilDone {
    #[new]
    fn new(conditions: Vec<PyObject>) -> PyResult<Self> {
        use pyo3::Python;
        let rust_conditions = Python::with_gil(|py| -> PyResult<Vec<RustStopCondition>> {
            conditions
                .iter()
                .map(|obj| {
                    let kind: String = obj
                        ...
                        .and_then(|v| v.extract::<String>(py))?;
                    match kind.as_str() {
                        "no_new_findings" => Ok(RustStopCondition::NoNewFindings),
                        "no_errors" => Ok(RustStopCondition::NoErrors),
                        "max_rounds" => {
                            let max: u32 = obj
                                .getattr(py, "max_rounds")
                                .and_then(|v| v.extract::<u32>(py))
                                .map_err(|_| {
                                    PyValueError::new_err(
                                        "max_rounds condition requires an int max_rounds",
                                    )
                                })?;
                            Ok(RustStopCondition::MaxRounds(max))
                        }
                        other => Err(PyValueError::new_err(format!(
                            "unknown stop condition kind: {other}"
                        ))),
                    }
                })
                .collect()
        ...
        Ok(Self {
            inner: RustLoopUntilDone::new(RustLoopConfig::new(rust_conditions)),
        })
    }

    fn start(&mut self) -> LoopAction {
        LoopAction::from_rust(self.inner.start())
    }

    fn feed(&mut self, report: RoundReport) -> LoopAction {
        LoopAction::from_rust(self.inner.feed(RustRoundReport {
...
fn _kernel(m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    // Workflow primitives
    m.add_class::<TournamentMatch>()?;
    m.add_class::<TournamentAction>()?;
    m.add_class::<Tournament>()?;
    m.add_class::<RoundReport>()?;
    m.add_class::<LoopAction>()?;
    m.add_class::<LoopUntilDone>()?;
    Ok(())
}
