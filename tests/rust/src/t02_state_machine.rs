...
fn user_tools_appear_in_call_llm() {
    ...
    sm.run_spec.as_mut().unwrap().exposure_baseline = Some(vec![CompactString::new("read_file")]);
...
fn user_tools_plus_skill_tool() {
    ...
    sm.run_spec.as_mut().unwrap().exposure_baseline = Some(vec![CompactString::new("search")]);
    sm.ctx
