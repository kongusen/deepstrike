...
- Runtime and bindings now accept only the Canonical Kernel ABI (wire revision 3). The v1/v2
  operations, direct-step helpers, adapters, negotiation and fallback paths have been removed.
- An unfinished 0.2.50 operation cannot resume under 0.2.51. Finish or cancel it before upgrading;
  deployments that must recover it must remain on 0.2.50. Session logs remain usable for audit but
  are not recovery authority. See [the 0.2.51 migration guide](MIGRATION-0.2.51.md).
- Recovery now requires the canonical `KernelJournal` and uses logical checkpoints plus a bounded
  journal tail. `SessionLog`, full snapshots and SDK-authored `resumed_*` facts cannot mint or repair
  a production operation.
- Host callers can no longer submit legacy lifecycle operations, self-declared caller/task identity,
  or `SkillActivated`. Root execution is `StartOperation(RootEntry::Agent | RootEntry::Workflow)`;
  effects complete only through their correlated `ResolveEffect` input.

### Changed

- Canonical workflow specs now preserve context inheritance through core, Node, Python and WASM.
  Fan-out workers retain system-only context while synthesis nodes receive full parent context;
  facade-level workflow rejection is surfaced as an error instead of an empty result.
- Unknown host effects now resolve with the original effect ID as non-retryable `ProtocolError`.
  WASM retains unknown kernel observations as opaque `kernel_observation` events instead of dropping
  them.
- Skill activation is owned by provider-result reduction in the kernel. Four-language runners no
  longer generate/replay `skill_activated`, and canonical adapters no longer consume it as a no-op;
  host knowledge loading/pinning and explicit deactivation remain.
- Durable records, digests, effect/task identity and logical checkpoints are generated by core and
  projected consistently through the Rust, Node, Python and WASM bindings.
