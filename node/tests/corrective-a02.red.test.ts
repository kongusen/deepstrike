...
import type {
  LLMProvider, ModelMessage, RenderedContext, StreamEvent, ToolCall, ToolOutputBlock, ToolResultEvent, ToolSchema,
} from "../src/types.js"
...
class TwoSessionProvider implements LLMProvider {
  ...
  private calls = 0
  async complete(): Promise<ModelMessage> { return { role: "assistant", content: "done" } }
  ...
  it("rejects provider files when endpoint affinity does not match", () => {
    ...
    context.turns[0].contentParts![0] = {
      ...(context.turns[0].contentParts![0] as Extract<
        NonNullable<ModelMessage["contentParts"]>[number],
        { type: "tool_result" }
      >),
      contentParts: [{
        type: "file",
