 ...
 * the EXACT decoded `ModelMessage` it returns for a fixed vendor response.
 *
...
import type { LLMProvider, ModelMessage } from "../../src/types.js"
...
describe("spc_013-A-00 characterization: wire request bodies + complete decode", () => {
  ...
  it("ollama", async () => {
    ...
    try {
      const captured: { req?: unknown } = {}
      stubOllama(captured)
      const message: ModelMessage = await provider.complete(CTX, TOOLS)
