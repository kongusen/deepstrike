 ...
 *
 * One gateway serves N peer loops: each loop claims with its own sessionId and drains only
...
async function claimAndAck(gateway: SignalGateway, recipient?: string): Promise<RuntimeSignal | null> {
  const claim = await gateway.claimSignal(recipient)
  if (!claim) return null
  expect(await gateway.ackSignal(claim)).toBe(true)
  return claim.signal
}
...
describe("SignalGateway recipient addressing (R1/L0)", () => {
  ...
  it("each loop drains only its own + shared signals from one shared gateway", async () => {
    ...
    const a1 = await claimAndAck(gw, "sess-a")
    const a2 = await claimAndAck(gw, "sess-a")
    expect([a1?.payload.goal, a2?.payload.goal].sort()).toEqual(["shared", "to-a"])
    expect(await claimAndAck(gw, "sess-a")).toBeNull()

    // sess-b's signal is still queued for its own puller.
    expect((await claimAndAck(gw, "sess-b"))?.payload.goal).toBe("to-b")
  })
  ...
  it("preserves FIFO order among a recipient's visible signals", async () => {
    ...
    expect((await claimAndAck(gw, "sess-a"))?.payload.goal).toBe("first")
    expect((await claimAndAck(gw, "sess-a"))?.payload.goal).toBe("second")
  })

  it("omitting recipient claims the shared FIFO in order", async () => {
    const gw = new SignalGateway()
    gw.ingest(sig("x", "sess-a"))
    gw.ingest(sig("y"))
    expect((await claimAndAck(gw))?.payload.goal).toBe("x")
    expect((await claimAndAck(gw))?.payload.goal).toBe("y")
    expect(await claimAndAck(gw)).toBeNull()
  })
  ...
  it("fans a broadcast out so every explicit recipient receives one copy", async () => {
    ...
    expect((await claimAndAck(gw, "sess-a"))?.payload.goal).toBe("all")
    expect((await claimAndAck(gw, "sess-b"))?.payload.goal).toBe("all")
  })
