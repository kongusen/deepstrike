...
/**
 * Node's canonical direct-step host.
 *
...
/**
 * Compatibility surface used only inside the Node host while its runner callsites are cut over.
        ...
        for (const value of Array.isArray(event.results) ? event.results : []) {
          ...
          if (bytes > this.payloadInlineThreshold && this.options.persistPayload) {
            ...
            this.hostObservations.push({
              ...
              original_size: bytes,
              preview_size: Buffer.byteLength(persisted.preview, "utf8"),
            })
            results.push({
              kind: "external",
              call_id: callId,
  ...
  private mergeLegacyConfig(config: Record<string, unknown>): void {
    ...
    if (config.reliability) {
      ...
      if (
        ...
        throw new Error("invalid_config: legacy replay capacity must be positive during Node cutover")
      }
