 ...
 */
export function rebuildKernelRuntime(
  runtime: KernelRuntimeHandle,
    ...
    try {
      if (prepared.status !== "prepared" || !token) {
        throw new KernelLogIntegrityError(
          `kernel rebuild step ${transaction.step_seq} was not accepted as a new transition`,
        )
      }
      if (prepared.base_generation !== transaction.base_generation) {
        throw new KernelLogIntegrityError(
          `kernel rebuild generation ${prepared.base_generation} does not match transaction ${transaction.base_generation}`,
        )
      }
      if (prepared.step.step_seq !== transaction.step_seq) {
        throw new KernelLogIntegrityError(
          `kernel rebuild step_seq ${prepared.step.step_seq} does not match transaction ${transaction.step_seq}`,
        )
      }
      if (kernelRecordDigest(prepared.input) !== transaction.input_digest) {
        throw new KernelLogIntegrityError(
          `kernel rebuild normalized input diverged at step ${transaction.step_seq}`,
        )
      }
      if (kernelRecordDigest(prepared.step) !== transaction.step_digest) {
        throw new KernelLogIntegrityError(
          `kernel rebuild planned step diverged at step ${transaction.step_seq}`,
        )
      }
      ...
      if (kernelRecordDigest(committed) !== transaction.step_digest) {
        throw new KernelLogIntegrityError(
          `kernel rebuild committed step diverged at step ${transaction.step_seq}`,
        )
      }
    } catch (error) {
...
function assertFreshRuntimeMatchesGenesis(
  runtime: KernelRuntimeHandle,
  genesis: KernelOperationGenesis,
): void {
