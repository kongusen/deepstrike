...
export interface EvolutionViolation { readonly code: string; readonly detail: string }
...
export function createEvolutionRuntimeAdapter(validateJson: EvolutionValidateJson) {
  return {
    validate(bundle: EvolutionBundle): EvolutionReport {
      return JSON.parse(validateJson(JSON.stringify(bundle))) as EvolutionReport
    },
  }
}
