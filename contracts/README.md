...
Skill sources share one public `SkillRef`/`Skill` model. `InlineSkillCatalog`, `DirectorySkillCatalog`, and `DatabaseSkillCatalog` are runtime resolvers; source selection, user scope, and storage never leak into the public Agent language.
