...
| `memory-recall`         | long-term memory (MemoryStore) | `memory-empty` / `memory-preloaded` | pre-seeded memory cuts `turnsToDone` **−57%**, `wallMs` −47%, `inputTokens` −65%, `dollars` −55% at preserved quality |
...
| 7.4 | memory / aspiration | `memory-recall` | ✅ shipped | pre-seeded `MemoryStore` vs. empty; uses the public `InMemoryMemoryStore` (promoted to SDK in v0.2.21) |
