# ADR-011: Bind Context Evidence to Evaluation Runtime
...
without proving which context policy, accepted input, rendered prompt, or token measurement produced
the result. That leaves a replay gap: a changed renderer or retrieval result could be hidden behind the
same operation and dataset identifiers.
