...
- `snapshot_input_limit`：`KernelSnapshot` 为确定性重建保留的已接受 ABI 事务上限。
...
`KernelSnapshot.initial_policy` 中的 64-bit budget 轴使用十进制字符串编码，避免 Node/WASM JSON 往返时发生 `Number` 精度丢失。
