...
- [x] fingerprint 未变时复用完成结果；从第一个变更、失败或缺失结果开始，后续 invocation 作为 suffix 重新执行。
- [x] 动态提交现在携带 typed plan（run、序号、节点身份、依赖、prompt fingerprint）进入 canonical kernel；kernel journal 记录 plan observation。每个 executed/reused invocation 也提交 replay fact（状态、termination、结果 digest、replay disposition），host replay store 继续保存可复用的结果正文。
