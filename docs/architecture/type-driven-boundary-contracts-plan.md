...
- [x] 增加 `HostCommand::AppendWorkflowNodes` 与 `CanonicalRunnerRuntime.appendWorkflowNodes()`，让动态控制器可以通过 host authority 追加到活动 root workflow；kernel 继续负责 DAG、quota、trust 和 spawn。
