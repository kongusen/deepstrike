...
**token 数字的分界线**：出现在 checkpoint 里 = 冻结记账锚（合法——restore 必须复现 budget 算术，即使 tokenizer 变了）；运行期消息不再承载 token projection。engine 重算、host `TokenMeasurement` 侧表与 usage/attempt evidence 各司其职。
