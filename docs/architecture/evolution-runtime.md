...
`EvaluationRun` 还必须为每个被评估的 operation 携带一个 `EvaluationContextBinding`。Binding 固定解析后的 context policy、canonical input snapshot、renderer projection、prompt measurement 和可选 cache prefix，并要求这些引用出现在 evaluation evidence 中。原始 context bytes 仍由 host 持有，详见 [Evaluation Context](./evaluation-context)。
...
Canonical runner 接受可选的 host `artifactSetDigest`。未绑定的 operation 可以省略；被 promotion 或 replay 的 artifact set 必须显式传入经过验证的 content-addressed digest。SDK 不再提供 bootstrap fallback。
