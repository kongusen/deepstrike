...
这意味着下一个 member 启动时会继承前面所有 member 的累计消耗。
...
| 实现 | 位置 | 适用场景 |
|------|------|----------|
| `InMemoryGroupBudgetStore` | Python / Node | 单进程开发、测试 |
| `SessionLogGroupBudgetStore` | Python / Node | 把 ledger 和 membership 写入 `SessionLog`，可跨 store instance 重建 |
