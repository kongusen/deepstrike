...
| Phase 1 / PR 2：Kernel ABI 固化 | 🟡 进行中 | Core ABI + Node/PyO3/WASM JSON ABI `KernelRuntime.step()` 已落地；Rust SDK Runner 已迁入 step 驱动；Node FFI 读侧 helpers 已补齐；Node/Python Runner 待迁移 |
...
7. [~] `RuntimeRunner` 重构为 input/action 驱动，不再散落 `sm.feed(...)` 细节（Rust SDK 已完成，Node/Python 待迁移）
8. [~] FFI 默认入口收口到 `KernelRuntime`，隐藏 `LoopStateMachine` / `ContextManager`（Node `KernelRuntime` 读侧 helper 已补齐）
