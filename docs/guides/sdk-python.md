...
outcome = await runner.run_workflow(WorkflowSpec(nodes=[
    WorkflowNodeSpec(task="规则:金额是整数分 —— 代码里有没有违反?", role="verify"),
    WorkflowNodeSpec(task="规则:所有 error 都向上传播 —— 有没有违反?", role="verify"),
    WorkflowNodeSpec(task="规则:时间戳都是 UTC —— 有没有违反?",       role="verify"),
    WorkflowNodeSpec(task="Skeptic:上面的 flag 里哪些是真违规?",      role="verify", depends_on=[0, 1, 2]),
]))
...
| `kind` | 行为 |
|---|---|
| spawn(默认) | 跑一次该节点的 agent |
| loop(`max_iters`) | 反复运行直到 agent 报告完成,以 `max_iters` 兜底 |
| classify(`branches`) | 分类器的结果选中一个分支,其余分支在运行前被剪掉 |
| tournament(`entrants`) | 生成 N 个参赛者,再跑两两对阵的 judge bracket 到一个冠军 |
| reduce(`reducer`) | **无 token 的宿主计算** —— 一个纯函数(`dedupe_lines` / `merge_json_arrays` / `concat` / `count`,或经 `reducers` 选项自定义)作用于依赖输出 |

模板构造器:`fanout_synthesize(workers, synth)`、`generate_and_filter(gens, filter)`、`verify_rules(rules, skeptic)`。

### 13.2 运行时扩展 DAG

把 `submit_workflow_nodes_tool` 交给某个节点,它的 agent 就能在运行中向活动 DAG 追加节点(真正的 loop-until-done;为发现的每条 claim 派一个 verifier)。提交的 `depends_on` 是**批次相对、仅向后**的;每个追加的 spawn 仍过同一道配额 / 深度 / 隔离门;提交会被记录并在 `resume_workflow` 时回放。

### 13.3 信任、schema 与预算

- **隔离无逃逸** —— 给读取不可信内容的节点设 `trust="quarantined"`;它申请可写隔离会在内核被拒,且它运行时提交的任何节点都被强制降级为 quarantined(污点传递,无权限升级)。
- **结构化输出** —— 给节点设 `output_schema`;runner 指示 agent、对结果做校验,不符合则带错误回喂、重跑一次。始终不符合的节点会失败(其依赖被饿死)。
- **预算即信号** —— 装上 `max_workflow_nodes` / `max_concurrent_subagents` 配额后,每个 spawn 出来的节点都会在 goal 里带上剩余余量,协调者据此定扇出规模。

### 13.4 恢复

`resume_workflow(spec)` 从 session log 恢复:已完成节点被跳过,运行时追加的节点被回放,内核从断点继续。
