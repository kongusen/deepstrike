...
code_refs:
  node: [AgentDefinition, AgentRunOptions, RunResult, LLMProvider]
  fields:
...
Node applications configure an executable Agent through `createAgent`; the facade manages the runtime internally. Low-level runtime controls live under `@deepstrike/sdk/advanced`.
