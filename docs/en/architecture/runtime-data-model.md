...
**The token-number dividing line**: in a checkpoint, a token count is a frozen accounting anchor (legal — a restore must reproduce the same budget arithmetic even if the tokenizer moved); runtime messages no longer carry a token projection. Engine recomputation, a host `TokenMeasurement` side table, and usage/attempt evidence remain separate authorities.
