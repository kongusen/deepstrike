...
Configuration is validated as a whole and applied atomically at the ABI boundary. Replay-window capacities are limited to `1..=65536`; the snapshot input limit is `1..=100000` with a default of `10000`; recovery attempts are at most `16`; and the spool preview must be nonzero and no larger than its threshold. Omitted fields keep kernel defaults.
