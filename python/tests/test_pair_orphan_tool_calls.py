...
def _user(text: str) -> ModelMessage:
    return ModelMessage(role="user", content=text, tool_calls=[])


def _asst(content: str, calls: list[tuple[str, str]]) -> ModelMessage:
    return ModelMessage(role="assistant", content=content,
                   tool_calls=[ToolCall(id=i, name=n, arguments="{}") for i, n in calls])


def _tool(call_id: str) -> ModelMessage:
    return ModelMessage(role="tool", content="", tool_calls=[],
