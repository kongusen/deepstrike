...
from deepstrike._kernel import ContentPartObj, ModelMessage, ToolSchema
...
def test_normalizer_returns_a_single_adapter_input_without_mutating_caller_fields() -> None:
    context = RenderedContext(turns=[ModelMessage(role="user", content="hello")])
...
def test_openai_serialization_rejects_media_without_a_source(part: ContentPartObj) -> None:
    context = RenderedContext(turns=[ModelMessage(role="user", content="", content_parts=[part])])
...
def test_responses_adapter_uses_the_same_media_validation_boundary() -> None:
    context = RenderedContext(turns=[ModelMessage(
        role="user",
        content="",
...
def test_pyo3_file_carrier_enforces_affinity_serializes_to_responses_and_refuses_kernel_wire() -> None:
    ...
    message = ModelMessage(role="user", content="", content_parts=[ContentPartObj(
        "file",
        file_id="file_1",
        provider_id="openai",
        endpoint_id="openai.responses",
