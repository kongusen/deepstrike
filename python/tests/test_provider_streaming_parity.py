...
async def test_openai_flushes_tool_calls_when_stream_ends_without_tool_finish_reason(monkeypatch):
    ...
    gen = provider.stream(RenderedContext(turns=[ModelMessage(role="user", content="hi")]), [])
...
async def test_ollama_provider_preserves_unterminated_final_ndjson_record(monkeypatch):
    ...
    events = [event async for event in provider.stream(RenderedContext(turns=[ModelMessage(role="user", content="hi")]), [])]
