...
async def test_run_workflow_rejected_submission_keeps_child_completion():
    """ABI: parent-request admission is independent of ChildCompleted."""
    class _SubmitOnceOrchestrator:
        def __init__(self):
            self.goals: list[str] = []
    ...
    class _Orch:
        ...
        async def run(self, ctx):
            ...
            return SubAgentResult(
                agent_id=ctx.spec.identity.agent_id,
                result=LoopResult(termination="completed", turns_used=1, total_tokens_used=1,
                                  final_message=ModelMessage(role="assistant", content='{"verdict":"pass"}')),
            )
    ...
    class _Orch:
        ...
        async def run(self, ctx):
            ...
            return SubAgentResult(
                agent_id=ctx.spec.identity.agent_id,
                result=LoopResult(termination="completed", turns_used=1, total_tokens_used=1,
                                  final_message=ModelMessage(role="assistant", content="never valid")),
            )
    ...
    class _Orch:
        ...
        async def run(self, ctx):
            ...
            return SubAgentResult(
                agent_id=ctx.spec.identity.agent_id,
                result=LoopResult(termination="completed", turns_used=1, total_tokens_used=1,
                                  final_message=ModelMessage(role="assistant", content="ok")),
            )
