...
class RenderedMessage:
  """Duck-typed stand-in for the pyo3 `_kernel.ModelMessage` on the provider-facing rendered
  path. The pyo3 ModelMessage constructor enforces `ContentPartObj` parts, so a structured
  ...
  `isinstance(msg, ModelMessage)` — duck-typing is sufficient and no provider is
