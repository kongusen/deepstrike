...
async def execute_tools(
    ...
    for call in calls:
        ...
        if tool is None:
            results.append(ToolExecutionResult(call_id=call.id, output=f"unknown tool: {call.name}", is_error=True))
            continue
        try:
            kwargs = json.loads(call.arguments)
            validation = validate_tool_arguments(tool.schema.parameters, kwargs)
            if validation.get("error"):
                results.append(ToolExecutionResult(call_id=call.id, output=f"invalid arguments: {validation['error']}", is_error=True))
                continue
            ...
            if isinstance(output, AsyncIterable):
                ...
                output = "".join(chunks)
            results.append(ToolExecutionResult(call_id=call.id, output=str(output)))
        except Exception as exc:
            results.append(ToolExecutionResult(call_id=call.id, output=format_tool_error(exc), is_error=True))
    return results
